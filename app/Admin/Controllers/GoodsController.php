...
class GoodsController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('goods_id', __('Goods id'))->sortable();
		$grid->column('cat_id', __('cat_name'))->display(function($cat_id){
		    return CategoryModel::find($cat_id)->cat_name;		
		});
        $grid->column('goods_sn', __('Goods sn'));
		$grid->column('goods_name', __('Goods name'))->display(function($name){
		   return "[".$name."]";
		});
		...
		$grid->column('is_delete', __('Is delete'))->display(function($status){
		    return $status ? '是': '否';
		});
